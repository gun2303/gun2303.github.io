...
const WalletView = ({ items, settings, onEditItem, tripId }) => {
  ...
  return (
    <div className="p-4 pb-28 space-y-6">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            <div className="flex justify-between text-sm text-slate-500 mb-1"><span>剩餘預算</span><span>{settings.budget > 0 ? ((totalSpent/settings.budget)*100).toFixed(0) : 0}%</span></div>
            ...
            <div className="h-2 w-full bg-slate-100 rounded-full mt-3 overflow-hidden">
                <div className="h-full bg-indigo-500 transition-all" style={{ width: `${Math.min(100, (totalSpent/(settings.budget||1))*100)}%` }}></div>
            </div>
        </div>
        <div className="bg-slate-800 rounded-2xl p-5 text-white">
            <div className="flex justify-between items-center mb-4">
                <span className="text-sm font-bold flex gap-1"><RefreshCw className="w-4 h-4"/> 快速換算</span>
                <select value={fromCurrency} onChange={e => setFromCurrency(e.target.value)} className="bg-white/20 rounded px-2 py-1 text-xs text-slate-800">
